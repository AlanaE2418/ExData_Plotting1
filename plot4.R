plot4<- function(){
    ...
    with(GAP_weekdays, plot(`Date/Time`, Global_active_power, 
    ...
    # plot up/right
    voltage<- formated_two_days %>%
        select("Date/Time", "Voltage")
    with(voltage, plot(`Date/Time`, Voltage, type = "l", 
                            ylab = "Voltage", xlab = "datetime", ygap.axis = 4))
    # plot down/right
    GRP<- formated_two_days %>%
        select("Date/Time", "Global_reactive_power")
    with(GRP, plot(`Date/Time`, Global_reactive_power, type = "l", 
